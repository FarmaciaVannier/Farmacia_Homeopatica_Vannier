document.addEventListener("DOMContentLoaded", () => {
  ...
  botonesAgregar.forEach((boton) => {
    boton.addEventListener("click", function () {
      ...
      // Obtener el precio directamente del texto del párrafo
      const precioSeleccionado = parseFloat(
        precioTexto.replace("Precio: $", "")
      );

      // Agregar el producto al carrito
      agregarProductoAlCarrito(nombreProducto, precioSeleccionado);
    });
  });
  ...
  function enviarCorreo(contactoInfo) {
    ...
    const data = {
      service_id: "service_qbcr9ci",
      template_id: "template_tl4ancl",
      user_id: "nbz2iGBEuBJu2CK3U",
      template_params: {
        username: contactoInfo, // Enviar el número de teléfono
  ...
  document.getElementById("checkout").addEventListener("click", () => {
    ...
    if (!phonePattern.test(contactoInfo)) {
